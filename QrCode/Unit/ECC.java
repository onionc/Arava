...
/**
 * 纠错码字 Data Correction Code words
 */
public class ECC {
     ...
     * @return
     */
    public static int[][][] getECC(String dataEncoding, int version, Data.LEVEL level){
        // 结果
        int eccData[][][]; 
        ...
        int flag = 0;
        // 根据每个分组下的块进行分组
        eccData = new int[groups][][]; // 初始化ecc数据数组
        eccData[0] = new int[g1Block][g1DcNum];
        for(int i=0; i<g1Block; i++){
            // System.out.printf("%d %d %d %d\n", dataEncoding.length(), flag, g1DcNum, flag+g1DcNum*8);
            eccData[0][i] = Common.getIntByStr(dataEncoding.substring(flag, flag+g1DcNum*8));
            flag += g1DcNum*8;
        }
        if(groups==2){ // 2 group
            eccData[1] = new int[g2Block][g2DcNum];
            for(int i=0; i<g2Block; i++){
                // System.out.printf("%d %d %d\n", flag, g2DcNum, flag+g2DcNum*8);
                eccData[1][i] = Common.getIntByStr(dataEncoding.substring(flag, flag+g2DcNum*8));
                flag += g2DcNum*8;
            }
        }
    ...
    public static void main(String[] args){
        ...
        int r[][][] = ECC.getECC(s1, 5, Data.LEVEL.Q);
        ...
        int r2[][][] = ECC.getECC(s2, 1, Data.LEVEL.M);
        ...
        int r3[][][] = ECC.getECC(s3, 1, Data.LEVEL.Q);
